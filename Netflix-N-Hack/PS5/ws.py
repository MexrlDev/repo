"""
Mobile WebSocket Secure Echo Server (PS5 Version)

Original by Earthonion

Ported to pure Python stdlib by MexrlDev
"""
...
async def read_frame(reader: StreamReader):
    """Read a single WebSocket frame. Returns (opcode, payload) or (None, None) on error."""
    try:
    ...
    try:
        while True:
            ...
            if opcode == 0x1:                     # text frame
                ...
                await send_frame(writer, payload, 0xA)   # pong
            # ignore other frames
...
async def main():
    ...
    print(f"listening to 0.0.0.0:1337 (wss://{local_ip}:1337)...")
    ...
    try:
        ...
        # Raised when the server is closed – normal shutdown
        pass
    except KeyboardInterrupt:
        # This will be caught by the outer handler, but we can also clean up here
